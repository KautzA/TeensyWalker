...
void GaitGen1(int Period, int Cycle, int Xtrans, int Ytrans, int Ztrans, float ZTurn){
  ...
  int Output1[NUM_LEGS][3] = {
    { //Leg 0
      (kInitialPositions[0][0] + Type1[0]),//X (Output[0][0])
      (kInitialPositions[0][1] + Type1[1]),//Y (Output[0][1])
      (kInitialPositions[0][2] + Type1[2]) //Z (Output[0][2])

      }
      ,
    { //Leg 1
      (kInitialPositions[1][0] + Type2[0]),//X
      (kInitialPositions[1][1] + Type2[1]),//Y
      (kInitialPositions[1][2] + Type2[2]) //Z
      }
      ,
    { //Leg 2
      (kInitialPositions[2][0] + Type1[0]),//X
      (kInitialPositions[2][1] + Type1[1]),//Y
      (kInitialPositions[2][2] + Type1[2]) //Z
      }
      ,
    { //Leg 3
      (kInitialPositions[3][0] + Type2[0]),//X
      (kInitialPositions[3][1] + Type2[1]),//Y
      (kInitialPositions[3][2] + Type2[2]) //Z
      }
      ,
    };
